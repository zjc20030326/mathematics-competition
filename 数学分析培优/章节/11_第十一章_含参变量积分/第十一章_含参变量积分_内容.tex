...
\begin{solution}
    ...
    \begin{enumerate}
        \item 当 $y\to0^+$ 时,$(1+xy)^{\frac{1}{y}}\to e^x$ 在 $x\in\lbrack0,1\rbrack$ 上一致成立,故
              \[
                  \displaystyle\lim\limits_{y\to0^+}\int_0^1\dfrac{\mathrm{d}x}{1+(1+xy)^{\frac{1}{y}}}
                  =\int_0^1\dfrac{\mathrm{d}x}{1+e^x}
                  =\ln\dfrac{2e}{1+e}.
              \]
        \item 被积函数及积分上下限均连续变化,故
              \[
                  \displaystyle\lim\limits_{a\to0}\int_a^{1+a}\dfrac{\mathrm{d}x}{1+a^2+x^2}
...
\begin{solution}
    ...
    于是
    \[
        \displaystyle\lim\limits_{t\to0^+}F(t)=\dfrac{\pi}{2}f(0),
        \qquad
...
\begin{solution}
    ...
    \[
        \displaystyle\int_0^{2\pi}e^{re^{i\theta}}\,\mathrm{d}\theta
...
\begin{proposition}[\markstar\ 端点法]
    ...
    \[
        \displaystyle\lim\limits_{y\to y_0}\int_A^{+\infty}f(x,y)\,\mathrm{d}x=B\ne0,
    \]
...
\begin{solution}
    ...
    \begin{enumerate}
        \item 对任意 $A>0$,取 $\alpha=A^{-2}$,则
              \[
                  \displaystyle\int_A^{+\infty}e^{-\alpha x^2}\,\mathrm{d}x
                  =A\int_1^{+\infty}e^{-t^2}\,\mathrm{d}t,
              \]
              不随 $A\to+\infty$ 而趋于 $0$.
        \item 令 $t=\sqrt\alpha x$,则
              \[
                  \displaystyle\int_A^{+\infty}\sqrt\alpha e^{-\alpha x^2}\,\mathrm{d}x
                  =\int_{A\sqrt\alpha}^{+\infty}e^{-t^2}\,\mathrm{d}t.
              \]
              对每个 $A$ 令 $\alpha\to0^+$,右端趋于 $\sqrt\pi/2$,故不满足一致 Cauchy 准则.
        \item 已知 Fourier 积分
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{x\sin(xy)}{1+x^2}\,\mathrm{d}x
                  =\dfrac{\pi}{2}e^{-y},
                  \qquad y>0.
              \]
...
\begin{solution}
    ...
    若 $0<y<2/A$,则
    \[
        \displaystyle\int_A^{+\infty}e^{-\frac{1}{y^2}(x-\frac{1}{y})^2}\,\mathrm{d}x
...
\begin{solution}
    ...
    \[
        \displaystyle\int_0^{+\infty}\dfrac{e^{-x}}{|\sin x|^b}\,\mathrm{d}x<+\infty.
    \]
...
\begin{theorem}[\markstar\ 连续性]
    ...
    \[
        \displaystyle\lim\limits_{t\to t_0}\int_a^{+\infty}f(x,t)\,\mathrm{d}x
...
\begin{theorem}[\markstar\ 积分换序]
    ...
    \[
        \displaystyle\int_\alpha^\beta\mathrm{d}t\int_a^{+\infty}f(x,t)\,\mathrm{d}x
...
\begin{remark}
    ...
    \[
        \displaystyle\int_\alpha^{+\infty}\mathrm{d}t\int_a^{+\infty}f(x,t)\,\mathrm{d}x,
        \qquad
...
\begin{solution}
    ...
    \[
        \displaystyle\int_A^B\dfrac{\sin(ux^2)}{x}\,\mathrm{d}x
...
\begin{example}
    ...
    \[
        \displaystyle\int_c^{+\infty}|\varphi(y)|\,\mathrm{d}y
    \]
    收敛, 则
    \[
...
\begin{solution}
    ...
    \[
        |f(x,y)-f(x_0,y)|<\dfrac{\varepsilon}{\displaystyle 1+\int_c^{+\infty}|\varphi(y)|\,\mathrm{d}y}.
    \]
    于是
    \[
        |F(x)-F(x_0)|
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item 设 $f(x)\in C(\mathbb{R})$ 且 $f(x)\geq0$, $\displaystyle\int_0^{+\infty}f(x)\,\mathrm{d}x$ 收敛, 则
              \[
                  \displaystyle\lim\limits_{n\to\infty}\int_0^{+\infty}n\ln\left(1+\dfrac{f(x)}{n}\right)\mathrm{d}x
...
\begin{solution}
    ...
    \begin{enumerate}
        \item
              \[
                  \displaystyle\int_0^y e^{-x}\,\mathrm{d}x=1-e^{-y}\longrightarrow1.
              \]
...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{a\to0^+}\int_0^{+\infty}e^{-ax}f(x)\,\mathrm{d}x
        =\int_0^{+\infty}f(x)\,\mathrm{d}x.
    \]
\end{example}
\exampleblank
...
\begin{example}[\markstar]
    ...
    \[
        \displaystyle\lim\limits_{\lambda\to+\infty}\int_0^A\dfrac{\sin\lambda x}{x}f(x)\,\mathrm{d}x
        =\dfrac{\pi}{2}f(0^+).
    \]
\end{example}
\exampleblank
...
\begin{solution}
    写成
    \[
        \displaystyle\int_0^A\dfrac{\sin\lambda x}{x}f(x)\,\mathrm{d}x
...
\begin{example}
    ...
    计算 Fresnel 积分
    \[
        \displaystyle\int_0^{+\infty}\sin x^2\,\mathrm{d}x.
    \]
\end{example}
\exampleblank
...
\begin{example}
    试利用
    \[
        \displaystyle\int_0^{+\infty}e^{-u^2}\,\mathrm{d}u
        \overset{u=\alpha x}{=}
...
\begin{example}
    ...
    \[
        \displaystyle\int_0^{+\infty}e^{-x}f(x)\,\mathrm{d}x
    \]
    收敛, 且
    \[
        \displaystyle\int_0^{+\infty}e^{-x}f(x)\,\mathrm{d}x
...
\begin{solution}
    ...
    单调增加到 $f(x)$.对每个 $N$,
    \[
        \displaystyle\int_0^{+\infty}e^{-x}f_N(x)\,\mathrm{d}x
...
\begin{solution}
    ...
    又
    \[
        \displaystyle\int_0^{\frac{\pi}{2}}\cos^{2n}\theta\,\mathrm{d}\theta
...
\begin{example}
    计算积分
    \[
        \displaystyle\int_0^{+\infty}\dfrac{x}{1+e^x}\,\mathrm{d}x.
    \]
\end{example}
\exampleblank
...
\begin{example}[\markstar]
    试证
    \[
        \displaystyle\int_0^{+\infty}\dfrac{x^{s-1}}{e^x+1}\,\mathrm{d}x
...
\begin{solution}
    ...
    \begin{enumerate}
        \item 令 $x=a+(b-a)t$,则
              \[
                  \displaystyle\int_a^b(x-a)^p(b-x)^q\,\mathrm{d}x
                  =(b-a)^{p+q+1}B(p+1,q+1).
              \]
        \item 由 $\displaystyle\int_0^{+\infty}\dfrac{x^{p-1}}{1+x^q}\,\mathrm{d}x
                  =\dfrac{\pi}{q}\csc\dfrac{p\pi}{q}$,取 $p=1,q=3$,得
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{\mathrm{d}x}{1+x^3}
                  =\dfrac{2\pi}{3\sqrt3}.
              \]
        \item 同理取 $p=1,q=4$,得
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{\mathrm{d}x}{1+x^4}
                  =\dfrac{\pi}{2\sqrt2}.
              \]
    \end{enumerate}
\end{solution}
...
\begin{example}
    ...
    \begin{enumerate}
        \item
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{x^{p-1}}{(a+bx^q)^r}\,\mathrm{d}x
                  =\dfrac{a^{\frac{p}{q}-r}}{q\,b^{\frac{p}{q}}}
                  B\left(\dfrac{p}{q},r-\dfrac{p}{q}\right),\qquad a,b>0;
              \]
        \item
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{\mathrm{d}x}{(x^2+1)^n}
...
\begin{solution}
    ...
    \begin{enumerate}
        ...
        \item 令 $t=x^2$,则
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{\mathrm{d}x}{(1+x^2)^n}
...
\begin{solution}
    ...
    \begin{enumerate}
        \item 令 $t=\sin^2x$,则
              \[
                  \displaystyle\int_0^{\frac{\pi}{2}}\sin^\alpha x\,\mathrm{d}x
        ...
        \item 令 $u=\cos x$,再作分式线性代换
              ...
              直接化简得
              \[
                  \displaystyle\int_0^\pi\dfrac{\sin^{n-1}x}{(1+k\cos x)^n}\,\mathrm{d}x
...
\begin{solution}
    ...
    \begin{enumerate}
        \item 对 $0<p<1$,记
              ...
              在任意紧子区间上可对参数求导,故
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{x^{p-1}\ln x}{1+x}\,\mathrm{d}x
        ...
        \item 对 $0<a<4$,记
              ...
              二次求导并取 $a=1$,得
              \[
                  \displaystyle\int_0^{+\infty}\dfrac{\ln^2x}{1+x^4}\,\mathrm{d}x
...
\begin{solution}
    ...
    因此
    \[
        \displaystyle\int_0^1\ln\Gamma(x)\,\mathrm{d}x=\dfrac{1}{2}\ln(2\pi).
    \]
    再次使用反射公式,并注意 $\sin\pi(1-x)=\sin\pi x$,有
    \[
        2\int_0^1\ln\Gamma(x)\sin\pi x\,\mathrm{d}x
        =\int_0^1\left[\ln\pi-\ln(\sin\pi x)\right]\sin\pi x\,\mathrm{d}x.
    \]
    由
    \[
    ...
    得
    \[
        \displaystyle\int_0^1\ln\Gamma(x)\sin\pi x\,\mathrm{d}x
...
\begin{example}[\markstar]
    设 $t>1$, 证明
    \[
        \displaystyle\int_1^{+\infty}\dfrac{(\ln x)^{t-1}}{x(x-1)}\,\mathrm{d}x
        =\Gamma(t)\xi(t),
    \]
    其中
    \[
...
\begin{solution}
    ...
    \[
        \displaystyle\int_0^{+\infty}\dfrac{u^{t-1}}{e^u-1}\,\mathrm{d}u
