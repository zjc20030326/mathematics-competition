...
\begin{example}
    ...
    \[
        M(x)=\sup\limits_{a\leq t\leq x}f(t), \qquad m(x)=\inf\limits_{a\leq t\leq x}f(t).
    \]
    则:
    \begin{enumerate}
...
\begin{solution}
    第 (1) 问按现有题面不成立.例如取 $f\left(\dfrac{1}{2}\right)=1$,其余点 $f=0$,则 $M$ 在 $\dfrac{1}{2}$ 处不左连续;取 $f\left(\dfrac{1}{2}\right)=-1$,其余点 $f=0$,则 $m$ 在该点不左连续.因此``$f$ 仅有界''不足以推出所述结论,需核对原始资料是否漏了单侧连续条件.
...
\begin{solution}
    本题按现有题面不成立.例如
    \[
        f(x)= \begin{cases} 0, & 0\leq x<\dfrac{1}{2},   \\ 1, & \dfrac{1}{2}\leq x\leq1 \end{cases}
    \]
    单调不减,且 $f(\lbrack0,1\rbrack)=\{0,1\}$ 是闭集,但 $f$ 在 $\dfrac{1}{2}$ 处不连续.若把条件改为``$f(\lbrack0,1\rbrack)$ 是区间''或``$f$ 具有介值性'',则可由单调函数的跳跃间断会漏掉一段值域推出连续.
\end{solution}
...
\begin{solution}
    由题设,$f^{-1}(x)=2x-f(x)$.若 $f$ 递减,左边递减而右边严格递增,矛盾;故 $f$ 递增.把 $x$ 换成 $f(x)$ 得 $f(f(x))=2f(x)-x$.
    归纳得对任意整数 $n$, $f^n(x)=x+n(f(x)-x)$.
...
\begin{solution}
    因 $\dfrac{T_1}{T_2}$ 为无理数,集合 $\{mT_1+nT_2:m,n\in\mathbb{Z}\}$
...
\begin{solution}
    ...
    \[
        f(x)\equiv0,\qquad g(x)=\sin x,\qquad T_1=\sqrt{2},\qquad T_2=2\pi.
    \]
...
\begin{solution}
    令 $y=0$ 得 $f(0)=0$,再由方程得 $f(-x)=-f(x)$.归纳得 $f(nx)=nf(x)$,进而对有理数 $r$ 有 $f(rx)=rf(x)$.取 $x=1$,记 $a=f(1)$,由有理数的稠密性和 $f$ 的连续性, $f(x)=ax,\,x\in\mathbb{R}$.
...
\begin{solution}
    令 $g(x)=f(x)-f(0)$.在原式中依次取 $y=0$,$y=-x$ 并反复使用中点关系,可得对任意二进有理数 $r$, $g(rx)=rg(x)$.
...
\begin{example}
    证明:满足方程
    \[
        f(x+y)=f(x)+f(y), \qquad \forall x,y>0,
    \]
...
\begin{solution}
    本题题面与结论不匹配.方程 $f(x+y)=f(x)+f(y)$ 是加法 Cauchy 方程;在连续条件下,其在正半轴上的解为 $f(x)=cx,\,x>0$,
...
\begin{solution}
    非零解满足 $f(0)=1$ 且 $f(-x)=f(x)$.固定 $y$,序列 $u_n=f(x+ny)$ 满足 $u_{n+1}-2f(y)u_n+u_{n-1}=0$.
...
\begin{solution}
    ...
    第 (2) 问只能推出 $f$ 在 $(a,b)$ 内连续:若 $L=\displaystyle\lim\limits_{t\to x}f(t)\ne f(x)$,介值性将迫使靠近 $x$ 的函数值取到 $L$ 与 $f(x)$ 之间的值,与极限定义矛盾.但题面并未给出端点的单侧极限,故``在闭区间上连续''不能推出.例如令 $f(0)=0$,$f(x)=\sin\dfrac{1}{x}$ $(0<x\leq1)$,它具有介值性且每个内点的极限存在,但在 $0$ 不连续.
\end{solution}
...
\begin{solution}
    ...
    第 (1) 问中 $\xi_n\in\lbrack0,\dfrac{1}{n}\rbrack$ 的位置限制按现有题面不成立.例如 $n=3$,取 $f(x)=\sin\left(2\pi x-\dfrac{2\pi}{15}\right)$,则 $f(0)=f(1)$,但 $f\left(x+\dfrac{1}{3}\right)=f(x)$ 在 $\lbrack0,\dfrac{1}{3}\rbrack$ 内无解.正确的常用结论是存在 $\xi_n\in\lbrack0,1-\dfrac{1}{n}\rbrack$:对 $h(x)=f\left(x+\dfrac{1}{n}\right)-f(x)$ 使用分点处的望远求和及介值定理即得.

    第 (2) 问令 $g(x)=f(x)-x$,则 $g(0)=g(1)=0$.对 $g$ 应用上述正确结论,得某个 $\xi_n\in\lbrack0,1-\dfrac{1}{n}\rbrack$ 满足 $g\left(\xi_n+\dfrac{1}{n}\right)=g(\xi_n)$,这等价于
    \[
        f\left(\xi_n+\dfrac{1}{n}\right)=f(\xi_n)+\dfrac{1}{n}.
    \]
\end{solution}
...
\begin{solution}
    只需证明 $T>0$ 的情形,$T<0$ 可换向,$T=0$ 显然.若不存在所求子列,则存在 $\varepsilon>0$ 和 $A$,使 $x>A$ 时 $|f(x+T)-f(x)|\geq\varepsilon$.
    差函数连续且在连通区间上不为零,故其符号恒定.沿等差数列累加得 $|f(A_0+mT)-f(A_0)|\geq m\varepsilon$,
    与 $f$ 有界矛盾.
\end{solution}
...
\begin{solution}
    把 $f$ 以周期 $n$ 连续延拓到实轴.对每个整数 $k=1,\ldots,n$,函数 $h_k(x)=f(x+k)-f(x)$
...
\begin{solution}
    第 (1) 题中 $|f'(x)|=|\sin2x|\leq1$,故一致连续.
    
    第 (2) 题取
    \[
        x_n=\sqrt{2n\pi},\qquad y_n=\sqrt{2n\pi+\dfrac{\pi}{2}},
    \]
    则 $|x_n-y_n|\to0$,但函数值之差为 $1$,故非一致连续.
    
    第 (3) 题由 $|\sqrt x-\sqrt y|\leq\sqrt{|x-y|}$
    知一致连续.
    
    第 (4) 题取 $x_n=\dfrac{1}{n},y_n=\dfrac{1}{n+1}$ 知非一致连续.
    
    第 (5) 题取 $x_n=n,y_n=n+\dfrac{1}{n}$ 知非一致连续.
\end{solution}
...
\begin{solution}
    第 (1) 题在 $0$ 附近无界,故非一致连续.第 (2) 题可在 $0$ 处补值为 $1$,从而在紧区间上一致连续.第 (3) 题取使 $\cos\dfrac{1}{x}$ 分别为 $1,-1$ 的两列点;第 (4) 题同理取使 $\cos\dfrac{\pi}{x}$ 分别为 $1,-1$ 的两列点,均得非一致连续.

    第 (5) 题在 $0$ 处可连续补值,在 $\lbrack1,+\infty)$ 上导数有界,故一致连续.第 (6) 题在 $(0,1)$ 上因 $\sin\dfrac{1}{x}$ 的等幅振荡而非一致连续;在 $(1,+\infty)$ 上导数有界,故一致连续.
\end{solution}
...
\begin{solution}
    第 (1) 题在 $0$ 处可连续补值,且对大 $x$ 有 $f(x)=x+O(1)$,$f'(x)=1+O\left(\dfrac{1}{x}\right)$;第 (2) 题对大 $|x|$ 有 $f(x)=|x|+O\left(\dfrac{1}{|x|}\right)$,导数有界;第 (3) 题的根式内为 $(x-1)^2(x+1)$,在 $x=\pm1$ 附近分别有 $\dfrac{1}{3}$ 或 $\dfrac{2}{3}$ 次 Hölder 估计,在无穷远处为 $x+O(1)$ 且导数有界.将实轴分成根点附近的有界区间与其补集,可知三个函数都一致连续.
\end{solution}
...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{n\to\infty}f(x+n)=0, \qquad \forall x\in\lbrack 0,+\infty).
    \]
    则
    \[
