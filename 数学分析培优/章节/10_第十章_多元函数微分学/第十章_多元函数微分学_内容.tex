...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{x\to x_0}f(x,y)=\psi(y),\qquad y\ne y_0,
    \]
    且存在 $\eta>0$, 当 $0<|x-x_0|<\eta$ 时, $\displaystyle\lim\limits_{y\to y_0}f(x,y)=\varphi(x)$ 关于 $x$ 一致, 证明
    \[
        \displaystyle\lim\limits_{x\to x_0}\lim\limits_{y\to y_0}f(x,y)
...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{(x,y)\to(0,0)}f(x,y)=0.
    \]
\end{example}
\exampleblank
...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{\substack{x\to0^+\\y\to y_0}}f(x,y)=\varphi(y_0),
    \]
...
\begin{example}
    ...
    \[
        \displaystyle\sum\limits_{i=1}^n\dfrac{\partial f(x_0,y_0)}{\partial l_i}=0.
    \]
\end{example}
\exampleblank

\begin{solution}
    可微性给出方向导数为 $\nabla f\cdot\bm l_i$.正多边形各顶点向量之和为零,故所求和为 $\nabla f\cdot\displaystyle\sum_i\bm l_i=0$.
\end{solution}
...
\begin{example}
    ...
    \[
        \displaystyle\sum\limits_{i=1}^3\left(\dfrac{\partial f}{\partial l_i}\right)^2
        =f_x^2+f_y^2+f_z^2,
    \]
    \[
        \displaystyle\sum\limits_{i=1}^3\dfrac{\partial^2f}{\partial l_i^2}
        =f_{xx}+f_{yy}+f_{zz}.
    \]
\end{example}
\exampleblank
...
\begin{solution}
    将 $l$ 在正交基中展开为 $l=\displaystyle\sum_i\cos(l,l_i)l_i$,再用方向导数对方向的线性性即可.
\end{solution}
...
对二元函数, 常通过
\[
    \displaystyle\lim\limits_{\substack{\Delta x\to0\\\Delta y\to0}}
...
\begin{example}
    ...
    \[
        \displaystyle\lim\limits_{x\to\bm{0}}\dfrac{\partial f(x)}{\partial x_i}=0,
    \]
    证明 $f$ 在 $\bm{0}$ 处可微.
\end{example}
\exampleblank
...
\begin{example}[\markstar]
    ...
    \[
        \displaystyle\sum\limits_{i=1}^na_ix_i
...
\begin{solution}
    若两范数非零,令 $A=(\displaystyle\sum a_i^p)^{1/p}$,$X=(\displaystyle\sum x_i^q)^{1/q}$.Young 不等式给出
