        ...
        \item 由
              ...
              \[
                  \begin{aligned}
                      \tan(\tan x)
                       & =
                      \tan x+\dfrac{1}{3}\tan^3x+O(\tan^5x) \\
                       & =
                      x+\dfrac{2}{3}x^3+O(x^5).
                  \end{aligned}
              \]
              同理,
              \[
                  \begin{aligned}
                      \sin(\sin x)
                       & =
                      \sin x-\dfrac{1}{6}\sin^3x+O(\sin^5x) \\
                       & =
                      x-\dfrac{1}{3}x^3+O(x^5).
                  \end{aligned}
              \]
              因此
              \[
...
\begin{solution}
    ...
    当 $n=1$ 时,若 $|x|<\dfrac{\delta}{a}$,则 $|ax|<\delta$,由函数方程
    \[
        f(ax)=bf(x)
    \]
    得
    ...
    给定 $\varepsilon>0$,由于 $b>1$,可取正整数 $n$ 使
    \[
        \dfrac{M}{b^n}<\varepsilon.
    \]
    再令
    \[
        \eta=\dfrac{\delta}{a^n}.
    ...
    当 $|x|<\eta$ 时,
    \[
        |f(x)|\leq\dfrac{M}{b^n}<\varepsilon.
    \]
    故
    \[
        \displaystyle\lim\limits_{x\to0}f(x)=0.
...
\begin{solution}
    \begin{enumerate}
        \item 由于 $0\leq b\leq1$,有 $b^2\leq b$,故
              ...
              若 $f(x_0)>x_0$,取 $y>x_0$ 且充分接近 $x_0$,使
              \[
                  y<f(x_0).
              \]
              由递增性,
              \[
                  f(y)\geq f(x_0)>y,
              \]
              从而 $y\in E$,与 $x_0=\sup E$ 矛盾.故
              \[
                  f(x_0)\leq x_0.
              \]

              若 $f(x_0)<x_0$,则可取 $\eta>0$,使
              \[
                  f(x_0)<x_0-\eta.
              \]
              对任意 $x\in(x_0-\eta,x_0)$,由递增性,
              \[
                  f(x)\leq f(x_0)<x_0-\eta<x,
              \]
              因而 $(x_0-\eta,x_0)\cap E=\varnothing$,这与 $x_0=\sup E$ 矛盾.故
              \[
                  f(x_0)\geq x_0.
              \]
              因此
        ...
        \item 由 $f(a)>a\geq0$ 及 $f$ 递增,可知 $f(x)>0$ 对所有 $x\in\lbrack a,b\rbrack$ 成立.令
              \[
                  g(x)=\sqrt{f(x)}.
              \]
              则 $g$ 在 $\lbrack a,b\rbrack$ 上递增.又因为 $0\leq a<b\leq1$,
              \[
                  f(a)>a\geq a^2,
              \]
              故
              ...
              \[
                  g(b)<b.
              \]
              对 $g$ 重复第 (1) 问的确界论证,存在 $x_1\in(a,b)$,使
              \[
                  g(x_1)=x_1.
              \]
              即
...
\begin{solution}
    ...
    \[
        h(x)=f(x)+g(x).
    \]
    则 $h$ 在 $\lbrack0,1\rbrack$ 上递增.令
    \[
        E=\{x\in\lbrack0,1\rbrack\mid f(x)>0\}.
    \]
    由 $f(0)>0$,知 $E\neq\varnothing$;由 $f(1)<0$,知 $1\notin E$.

    先说明 $E$ 中有正数.由 $g$ 在 $0$ 点连续,当 $x>0$ 且充分小时,
    \[
        g(x)-g(0)<f(0).
    \]
    又由 $h(x)\geq h(0)$,
    ...
    同理,由 $g$ 在 $1$ 点连续,当 $x<1$ 且充分接近 $1$ 时,
    \[
        g(1)-g(x)<-f(1).
    \]
    又由 $h(x)\leq h(1)$,
    ...
    \[
        f(\xi)
        \geq f(x_n)+g(x_n)-g(\xi).
    \]
    由 $f(x_n)>0$ 及 $g(x_n)\to g(\xi)$,可得
    \[
        f(\xi)\geq0.
    \]

    若 $f(\xi)>0$,由 $g$ 在 $\xi$ 点连续,可取 $y>\xi$ 且充分接近 $\xi$,使
    \[
        g(y)-g(\xi)<f(\xi).
    \]
    又因 $h(y)\geq h(\xi)$,
...
\begin{solution}
    ...
    \[
        ...
        y\in U_x.
    \]
    开区间族 $\{U_x\}_{x\in K}$ 覆盖 $K$.由 Heine--Borel 有限覆盖定理,可从中取出有限个
    \[
        U_{x_1},U_{x_2},\ldots,U_{x_m}
    \]
    仍覆盖 $K$.令
    \[
        M=\max\{f(x_1),f(x_2),\ldots,f(x_m)\}.
    \]
    对任意 $y\in K$,存在某个 $i$ 使 $y\in U_{x_i}$,故
    \[
        f(y)\leq f(x_i)\leq M.
    \]
    ...
    \[
        ...
        x_j\to x\in K_n,
    \]
    则由题设在 $x$ 点存在邻域 $U$,使 $f(y)\leq f(x)$ 对 $y\in U$ 成立.当 $j$ 充分大时,$x_j\in U$,故
    \[
        M_n=f(x_j)\leq f(x)\leq M_n.
    \]
    ...
    \[
        ...
        x\in U_*.
    \]
    由于 $|K_n|\to0$ 且 $x_*\in K_n$,当 $n$ 充分大时有
    \[
        K_n\subseteq U_*.
    \]
    因此 $x_*$ 是 $f$ 在 $K_n$ 上的最大值点,即
    \[
        x_*\in A_n.
    \]
    但另一方面,
    \[
        x_*\in K_{n+1}\subseteq K_n\setminus A_n,
    \]
    矛盾.
