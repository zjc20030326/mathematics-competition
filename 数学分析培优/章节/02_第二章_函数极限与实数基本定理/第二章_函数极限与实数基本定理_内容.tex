...
事实上,在常见的初等函数极限问题中,许多可用洛必达法则处理的问题也可以通过 Taylor 展开解决,但两者的适用条件并不完全相同.实际计算中应根据表达式的结构选择更简洁的方法.一个简单的判别方法(但不绝对):若所给函数求导后的结果相当复杂,就应优先考虑 Taylor 公式,如复合了一堆函数.
...
\begin{solution}
    ...
    \]
    先说明 $A$ 为有限实数.任取 $y_0\in\mathring{U}_{+}(x_0)$.对任意
    $x\in\mathring{U}_{-}(x_0)$,由单调递增性有 $f(x)\leq f(y_0)$,故左侧函数值集合有上界,从而 $A\in\mathbb{R}$.

    给定 $\varepsilon>0$,由上确界定义可取 $x_1<x_0$,使
    \[
    ...
    \]
    任取 $y_1\in\mathring{U}_{-}(x_0)$.对任意 $x\in\mathring{U}_{+}(x_0)$,有 $f(x)\geq f(y_1)$,
    故右侧函数值集合有下界,从而 $B\in\mathbb{R}$.给定 $\varepsilon>0$,由下确界定义可取 $x_2>x_0$,使
    \[
    ...
    \[
        f(x_0+0)=B
        =\inf\limits_{x\in\mathring{U}_{+}(x_0)}f(x).
    \]
\end{solution}
...
\begin{theorem}[\markstar\ 函数版本 Stolz]
    ...
    \begin{enumerate}
        ...
        \item 对任意 $x>a$,有 $g(x+T)>g(x)$,且
              $\displaystyle\lim\limits_{x\to+\infty}g(x)=+\infty$.
    \end{enumerate}
    若极限
    \[
        \lim\limits_{x\to+\infty}
...
\begin{solution}
    先设 $A\in\mathbb{R}$.给定 $\varepsilon>0$,取 $X>a$,使当 $x>X$ 时,
    \[
        A-\varepsilon<
        \dfrac{f(x+T)-f(x)}{g(x+T)-g(x)}
        <A+\varepsilon.
    ...
    对任意充分大的 $x$,可写成
    \[
        x=y+nT,\qquad y\in\lbrack X,X+T),\qquad n\in\mathbb{N}.
    \]
    将上述不等式依次用于 $y,y+T,\ldots,y+(n-1)T$,并利用
    $g(z+T)-g(z)>0$,相加得
    \[
        ...
        <(A+\varepsilon)[g(x)-g(y)].
    \]
    因为 $f,g$ 在 $\lbrack X,X+T\rbrack$ 上有界,存在常数 $M>0$,使
    \[
        |f(y)|\leq M,\qquad |g(y)|\leq M,
        \qquad y\in\lbrack X,X+T).
    \]
    又因为 $g(x)\to+\infty$,故充分大的 $x$ 满足 $g(x)>0$.将上式除以
    $g(x)$,再令 $x\to+\infty$,得到
    \[
        A-\varepsilon
        \leq
        \liminf\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}
        \leq
        \limsup\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}
        \leq
        A+\varepsilon.
    \]
    再令 $\varepsilon\to0$,即得
    ...
    下面设 $A=+\infty$.任取 $K>0$,存在 $X>a$,使当 $x>X$ 时,
    \[
        \dfrac{f(x+T)-f(x)}{g(x+T)-g(x)}>K.
    \]
    仍将任意充分大的 $x$ 写成
    $x=y+nT$,其中 $y\in\lbrack X,X+T)$.
    对 $y,y+T,\ldots,y+(n-1)T$ 逐项应用上述不等式并相加,得到
    \[
        f(x)-f(y)>K[g(x)-g(y)].
    \]
    因而
    \[
        ...
        +\dfrac{f(y)-Kg(y)}{g(x)}.
    \]
    由于 $f(y),g(y)$ 在 $\lbrack X,X+T\rbrack$ 上一致有界,而
    $g(x)\to+\infty$,故
    \[
        \liminf\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}\geq K.
    \]
    $K>0$ 任意,所以
    \[
        \lim\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}=+\infty.
    \]
    最后设 $A=-\infty$.任取 $K>0$,充分大的 $x$ 满足
    \[
        \dfrac{f(x+T)-f(x)}{g(x+T)-g(x)}<-K.
    \]
    同样逐项相加可得
    ...
    令 $x\to+\infty$,得到
    \[
        \limsup\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}\leq-K.
    \]
    因 $K>0$ 任意,故
    \[
        \lim\limits_{x\to+\infty}\dfrac{f(x)}{g(x)}=-\infty.
...
%例题2.3.3
\begin{example}
    若 $f(x)$ 在 $\lbrack a,b\rbrack$ 无界,试证明存在 $x_0\in\lbrack a,b\rbrack$,对任意 $\delta>0$,使得 $f(x)$ 在 $(x_0-\delta,x_0+\delta)\cap\lbrack a,b\rbrack$ 上无界.
\end{example}
\exampleblank
    ...
    \begin{enumerate}
        \item 由于 $0\leq b\leq1$,有 $b^2\leq b$,故
              ...
              \[
                  f(y)\geq f(a)>y,
              \]
              故 $y\in E$,从而 $x_0>a$.若 $x_0=b$,由 $f(b)<b$,可取 $x<b$ 且充分接近 $b$,使 $f(b)<x<b$.
              对任意 $t\in\lbrack x,b\rbrack$,由递增性,
              \[
                  f(t)\leq f(b)<x\leq t,
              \]
              因而 $E\cap\lbrack x,b\rbrack=\varnothing$,从而 $\sup E\leq x<b$,与 $x_0=b=\sup E$ 矛盾.故 $x_0<b$.

              若 $f(x_0)>x_0$,可取 $y\in(x_0,b)$ 且充分接近 $x_0$,使 $y<f(x_0)$.由递增性,
              $f(y)\geq f(x_0)>y$,从而 $y\in E$,与 $x_0=\sup E$ 矛盾.故 $f(x_0)\leq x_0$.

              若 $f(x_0)<x_0$,则可取 $\eta>0$,使 $\eta<x_0-a$ 且 $f(x_0)<x_0-\eta$.对任意
              $x\in(x_0-\eta,x_0)$,由递增性,
              \[
                  f(x)\leq f(x_0)<x_0-\eta<x,
              \]
              因而 $(x_0-\eta,x_0)\cap E=\varnothing$,这与 $x_0=\sup E$ 矛盾.故 $f(x_0)\geq x_0$.因此 $f(x_0)=x_0$.
