...
\begin{solution}
    \begin{enumerate}
        \item 因
              ...
              逐项积分得到
              \[
                  \sum\limits_{n=1}^{\infty}\left[\dfrac{1}{n}-\ln\left(1+\dfrac{1}{n}\right)\right]=\gamma.
              \]
        \item 逐项积分得
              \[
                  \sum\limits_{n=1}^{\infty}(2^{-n}-5^{-n})=1-\dfrac{1}{4}=\dfrac{3}{4}.
              \]
...
\begin{solution}
    ...
    \[
        \sum\limits_{k=0}^{\infty}\dfrac{1}{2k+1}=+\infty,
    \]
...
\begin{solution}
    ...
    $\limsup\sqrt[n]{a_n}\leq1$,从而 $R\geq1$;而在 $x=1$ 时
    $\sum a_n=+\infty$,故 $R\leq1$.所以 $R=1$.
\end{solution}
...
\begin{solution}
    ...
    $\limsup\sqrt[n]{S_n}\geq1$,故半径不超过 $1$.于是仍为 $1$.
\end{solution}
...
\begin{solution}
    利用母函数
    \[
        \sum\limits_{k=0}^{\infty}C_{2k}^{k}x^k=(1-4x)^{-1/2}.
    \]
    两边平方并比较 $x^n$ 的系数:
    \[
        \sum\limits_{k+j=n}C_{2k}^{k}C_{2j}^{j}
            =[x^n](1-4x)^{-1}=4^n.
    \]
\end{solution}
...
\begin{solution}
    ...
    $\sum1/H_n$ 发散,$x=-1$ 时由 Leibniz 判别法收敛.因此收敛域为 $\lbrack-1,1)$.
...
\begin{solution}
    ...
    \[
        \limsup\limits_{n\to\infty}|a_n|^{2/n}=1,
    \]
    故 $\limsup|a_n/n|^{1/n}=1$,第二级数半径也为 $1$.又 $\sum a_n^2$ 收敛,由 Cauchy--Schwarz 不等式
    \[
        \sum\limits_{n=1}^{\infty}\dfrac{|a_n|}{n}
...
\begin{solution}
    记 $s_n=\sum_{k=1}^na_k$,Cesaro 可求和意味着
    ...
    $\sum |a_nx^n|$ 被 $\sum Cn|x|^n$ 控制,故绝对收敛.
\end{solution}
...
\begin{solution}
    因 $a_n>0$,部分和 $S_N=\sum_{n=0}^Na_n$ 单调增加.对任意固定 $N$,
    \[
        \sum\limits_{n=0}^Na_nx^n\leq\sum\limits_{n=0}^{\infty}a_nx^n.
    \]
    令 $x\to1^-$ 得 $S_N\leq S$.再令 $N\to\infty$,得到
    $\sum a_n\leq S$.反向不等式由 $x^n\leq1$ 得到,故 $\sum a_n=S$.
\end{solution}
...
\begin{solution}
    记 $s_n=\sum_{k=0}^na_k$.分部求和可把 $s_n-l$ 写成 Abel 平均误差与
    $n^{-1}\sum_{k=1}^nka_k$ 的线性组合.具体取 $x=1-1/n$,有
    \[
        s_n-f(x)=\sum\limits_{k=0}^na_k(1-x^k)-\sum\limits_{k>n}a_kx^k.
    \]
    对两项作 Abel 变换,并用
    $\frac{1}{n}\sum_{k=1}^nka_k\to0$,可得右端趋于零.又 $f(1-1/n)\to l$,故 $s_n\to l$.这正是题设 Tauber 条件下的结论.
\end{solution}
...
\begin{solution}
    利用 $1/(i+j)=\int_0^1x^{i+j-1}\,\mathrm{d}x$,有限双和等于
    \[
        \int_0^1\dfrac{1}{x}
    ...
    \[
        \int_0^1\dfrac{x}{(1+x)^2}\,\mathrm{d}x=\ln2-\dfrac{1}{2}.
    \]
\end{solution}
...
\begin{solution}
    ...
    $(1-x)\sum lx^n=lx\to l$.对余项,给定 $\varepsilon>0$,取 $N$ 使
...
\begin{solution}
    第 (1) 问由 $\ln(1+x)=\sum_{n\geq1}(-1)^{n-1}x^n/n$ 逐项除以 $x$ 并积分,得到交错平方和 $\pi^2/12$.

    第 (2) 问为第一类完全椭圆积分.由二项式级数
    \[
        (1-u)^{-1/2}=\sum\limits_{n=0}^{\infty}\dfrac{C_{2n}^{n}}{4^n}u^n
    \]
    及 $\int_0^{\pi/2}\sin^{2n}\theta\,\mathrm{d}\theta
        =\frac{\pi}{2}\frac{C_{2n}^{n}}{4^n}$,得
    \[
        \int_0^{\pi/2}\dfrac{\mathrm{d}\theta}{\sqrt{1-k^2\sin^2\theta}}
...
\begin{solution}
    ...
    $\sum 2^{-k\alpha}\cos(2^kx)$ 属于 $\operatorname{Lip}\alpha$,但在 $n=2^k$ 处有
...
\begin{solution}
    ...
    \[
        \sum\limits_{n=1}^{\infty}c_n\sin(nx)\asymp\dfrac{c_{\lfloor1/x\rfloor}}x,qquad x\to0^+.
    \]
...
\begin{solution}
    ...
    \]
    左端为 $61\pi^4/40$,再用 $\sum n^{-2}=\pi^2/6$,解得
    $\sum n^{-4}=\pi^4/90$.
\end{solution}
...
\begin{solution}
    ...
    \[
        \int_0^\pi|f|^2\,\mathrm{d}x=\dfrac{\pi}{2}\sum\limits_{n\geq1}a_n^2,qquad
    ...
    \[
        \int_0^l|f'|^2\,\mathrm{d}x\geq\dfrac{4\pi^2}{l^2}
