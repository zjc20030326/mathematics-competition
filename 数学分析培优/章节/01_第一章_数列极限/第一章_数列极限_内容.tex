...
\begin{solution}
  ...
  \[
    ...
    =\dfrac{|x_n-a|}
    {x_n^{2/3}+\sqrt[3]{x_n}\sqrt[3]{a}+a^{2/3}}
    \leq\dfrac{|x_n-a|}{|a|^{2/3}}.
  \]
...
\begin{solution}
  任给 $\varepsilon\in(0,1)$.由 $f(x)/x\to1$,存在 $\delta>0$,使得
  $0<x<\delta$ 时
  \[
...
\]
  ...
  \]
  因 $\displaystyle\max_{1\leq i\leq n}\alpha_{i,n}=1/n\to0$,由上面的结论,
  \[
    \lim\limits_{n\to\infty}\sum\limits_{i=1}^n
...
\begin{solution}
  ...
  \]
  取 $q\in(1/3,1)$.当 $n$ 充分大时 $x_{n+1}\leq qx_n$,故
  $x_n\leq Cq^n\to0$.
\end{solution}
...
\begin{solution}
  ...
  $x-3-x^{-1/2}$ 在 $(0,+\infty)$ 上严格递增,且在 $3$ 处为负、在 $4$ 处为正,故方程
...
\begin{solution}
  ...
  $x_{n+1}=-\dfrac{p}{q}x_n+\dfrac{y_n}{q}$ 迭代可知
  \[
    |x_n|\leq\left|\dfrac{p}{q}\right|^{n-1}|x_1|
    +\dfrac{\displaystyle\sup\limits_k|y_k|}{|q|}
     \sum\limits_{j=0}^{n-2}\left|\dfrac{p}{q}\right|^j,
  \]
  故 $\{x_n\}$ 有界.再由
  \[
    x_{n+1}-\dfrac{Y}{p+q}
    =-\dfrac{p}{q}\left(x_n-\dfrac{Y}{p+q}\right)
    +\dfrac{y_n-Y}{q},
  \]
  取绝对值的上极限,得到
  \[
    \varlimsup\limits_{n\to\infty}
    \left|x_n-\dfrac{Y}{p+q}\right|
    \leq\left|\dfrac{p}{q}\right|
    \varlimsup\limits_{n\to\infty}
    \left|x_n-\dfrac{Y}{p+q}\right|.
  \]
  因 $\left|\dfrac{p}{q}\right|<1$,上述有限的非负上极限只能为 $0$,故
  $x_n\to\dfrac{Y}{p+q}$.
  因此 $\{x_n\}$ 与 $\{y_n\}$ 同时收敛或同时发散.
\end{solution}

可以，精简后用“次可加数列 + 对数化”处理第二问会更干净。下面所有环境都不带星号，并按你的分式规范处理：
...
\begin{solution}
  先证 (2).若结论不成立,则可取 $q<1$,使得当 $n$ 充分大时
  \[
  ...
  \]
  右端最终为负,与 $a_{m+1}>0$ 矛盾,故 (2) 成立.

  对正数列 $\dfrac{a_n}{a_1}$ 应用 (2),得到
  \[
    \varlimsup\limits_{n\to\infty}
    n\left(\dfrac{a_1+a_{n+1}}{a_n}-1\right)\geq1.
  \]
  若 (1) 左端小于 $e$,则存在 $c<1$,使得当 $n$ 充分大时
  \[
