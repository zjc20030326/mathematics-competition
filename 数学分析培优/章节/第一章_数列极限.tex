...
\left(
\binom{n}{0}a_0+\binom{n}{1}a_1+\cdots+\binom{n}{k}a_k+\cdots+\binom{n}{n}a_n
\right)=a.
\]
\end{example}
\exampleblank
...
设
\[
S_n=\dfrac{\sum\limits_{k=0}^n\ln\binom{n}{k}}{n^2},
\]
...
\begin{example}
已知连续函数 $f(x)$ 在 $[1,+\infty)$ 单调递减,且 $f(x)>0$,证明数列
\[
...
\begin{corollary}[\markstartriangle]
...
\begin{proposition}[\marktriangle]
已知数列 $\{x_n\}$ 在区间 $I$ 上由
\[
...
\begin{proposition}[\markstartriangle]
设 $\{a_n\}$ 为正数列,则有
\[
