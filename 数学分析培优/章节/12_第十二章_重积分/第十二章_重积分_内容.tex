...
\begin{solution}
    ...
    \[
        ...
        +\int_1^2\bigl(e^x-\sqrt{x}\bigr)\dfrac{\ln x}{e^x}\,\mathrm{d}x
        +\int_2^{e^2}\bigl(e-\sqrt{x}\bigr)\dfrac{\ln x}{e^x}\,\mathrm{d}x.
    \]
...
\begin{solution}
    写成累次积分并把关于 $x$ 的对称项配对:
    \[
        I=\int_{-1}^1\!\mathrm{d}x\int_{x^3}^1x\bigl(1+yf(x^2+y^2)\bigr)\,\mathrm{d}y.
    \]
    其中
    \[
...
\begin{solution}
    先把内层积分写成差分:
    \[
        F'(x)=\dfrac{1}{h^2}\int_0^h\bigl[f(x+y+h)-f(x+y)\bigr]\,\mathrm{d}y.
    \]
    再次使用微积分基本定理,得到
    \[
...
\begin{solution}
    直接对 $z$ 积分:
    \[
        V=\int_0^1\!\mathrm{d}x\int_0^x\bigl(e^{x+y}-x-y\bigr)\,\mathrm{d}y
...
\begin{solution}
    ...
    \[
        I=\pi\int_0^{\frac{a}{2}}z^2\bigl[a^2-(z-a)^2\bigr]\,\mathrm{d}z
...
\begin{solution}
    \begin{enumerate}
        \item 大球在 $z\geq0$ 内的部分减去内切小球,按水平截面积分可得
              \[
                  \dfrac{\pi}{2}\int_0^4\left(\bigl[9-(z-1)^2\bigr]^2-\bigl[4-(z-2)^2\bigr]^2\right)\mathrm{d}z
                  =\dfrac{256\pi}{3}.
              \]
