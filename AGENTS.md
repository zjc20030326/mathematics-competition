...
```text
数学分析培优讲义(答案版).tex
章节/第X章_章名/第X章_章名(答案版).tex
```

答案版遵循以下规则：

1. 使用独立的答案版主文件和十三个答案版章节文件，并继续采用 `subfiles`。
2. 答案版章节只保留 `chapter`、`section`、`subsection`、`methodsection`、例题及其解答。
3. 不复制定义、定理、命题、引理、推论、注记和普通讲解正文，避免正文需要在两个版本中重复维护。
4. 每个 `example` 后直接紧跟对应解答，不添加 `\exampleblank`。
5. 原留白版仍须在每个 `example` 后保留 `\exampleblank`；答案版是本条规则的唯一例外。
6. `section`、`subsection`、`methodsection` 的分页规则与原讲义一致，但不得因删除正文而制造只有章标题的空白页。
7. 解答优先使用例题所属节或方法标题中已经介绍的方法；若原题按现有题面不成立，应给出反例并标明需要核对原始资料，不得擅自改题。
8. 答案章节必须支持单章编译，答案版主文件必须支持全书编译。
