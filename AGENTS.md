...
```text
CMC历年真题/
source/          （仅本地保存，不纳入 Git）
```

其中 `CMC历年真题/` 主要作为竞赛资料库使用，`source/` 保存后续整理讲义时需要对照的原始资料。

`source/` 中是第三方版权资料，只在本地保存，不提交到 Git，也不推送到远端（见 §4.2）。
...
### 4.2 `source/`（仅本地，不纳入 Git）

`source/` 是原始参考资料目录，保存的是第三方版权资料（打印版讲义、参考书等）。

规则：

- 不纳入 Git：该目录已在 `.gitignore` 中忽略，不提交、不推送、不加入索引；
  本地文件继续保留，仅供查阅与校对；
- 若发现它曾被跟踪，用 `git rm -r --cached source/` 取消跟踪，不要删除本地文件；
- 原始 PDF 只用于查阅、校对和溯源；
- 不擅自修改、压缩、覆盖、移动或重命名；
...
### 6.6 数学公式排版：行间公式与 aligned 使用规范

1. 行间公式不是默认选择。

   对于简短公式、简单极限、简单等式关系，应优先使用行内公式。

   不要为了公式独立显示而强制创建行间公式。

2. 短等式链不要滥用 aligned。

   如果一个等式链在页面宽度内可以正常显示，应优先写成：
...
4. 不要为了 LaTeX 源码整齐提前拆行。

   数学讲义排版优先考虑最终 PDF 的视觉效果。

   允许源码中存在较长公式，只要最终 PDF 不产生明显溢出。
...
6. 行内公式内的间距用小间距。
   ...
   行间公式行内化后，公式内的 `\qquad`、`\quad` 等显示级分隔符不要一并带入正文。

   行内公式里分隔并列项或条件限定语统一使用 `\,`，不使用 `\quad`、`\qquad`，也不使用 `\ `。例如：

   ```latex
   $\displaystyle a_n=\sqrt[n]{1+b^n}\,(b>0)$
   $x_0(1-x_0),\,x_0(1+x_0)$
   ```

   本规则对数学分析培优与高等代数培优的全部章节正文同时生效。

7. 「公式—承接语—公式」之间不要留空行。

   两个行间公式属于同一连续推导，中间只有一句或一小段用于连接前后公式的文字时
   （如「由此可得」「因此」「于是」「从而」「代入得」「又因为」「故」「即」等），
   这段文字与前后公式之间不要留空行，不要让它形成新的首行缩进段落。

   但如果中间的文字是在开启新的论述、解释、方法、编号项或独立段落，
   则应保留空行。

   不要机械地把「两个行间公式之间只有文字」当作删空行的条件。
...
`theorem`、`proposition`、`corollary`、`solution` 等环境的标题是 run-in 样式，
正文若直接以列表或手写编号开始，标题会与第一项同行、导致各项错位。两种处理：

- 正文直接以 `enumerate`、`itemize` 列表开始时，在列表前使用 `\leavevmode`：
...
- 正文直接以手写编号 `(1)`、`(i)` 等开始时，不用 `\leavevmode`，
  而是在编号前留一个空行：
...
改动共享文件后必须同步另一本，不要只改其中一本；是否编译验证见 §15。
...
- 不擅自改题；
- 不静默“修正”成模型认为正确的版本；
- 优先对照 `source/` 中原始资料（本地资料，不纳入 Git）；
- 不能确认时保留当前内容并明确标记待核对；
...
默认只做静态检查，不编译：修改完成后运行 `writing-math-competition-lectures` Skill
的 validator（`check --scope changed`），只报告源码层面的结论，不要主动编译，也不要主动
做 PDF 目视检查。

只有用户明确要求编译验证（例如「编译一下」「编译检查」「看下 PDF」）时，才按下表执行。
...
但凡改动 `elegantbook.cls`、`preamble.tex` 或任一主文件，两本都要同步修改，并在被要求编译时分别验证。
...
- 避免因预期的非零退出码产生无意义的失败和重试。
- `grep`/`rg` 无匹配、`git diff --quiet` 有差异等情况不是执行错误，应显式处理。
- 不要把可能正常返回非零状态的检查命令直接串在 `&&` 链中。
- 已有可靠 validator 或最新 clean build 覆盖的检查，不要重复执行等价的 grep/扫描。
- 提交前检查应尽量批量完成，减少多个串行 tool call。
- Git 提交流程优先采用：
  1. `git status --short`
  2. 必要的项目 validator / build check
  3. `git add` 指定文件
  4. `git diff --cached --check` 和 staged diff 检查
  5. `git commit`
  6. `git push`
- 除非前一步发现异常，否则不要增加重复的 sanity-check。

这不会让它“少检查到不安全”，而是把重复检查和假失败去掉。
