...
\]
若 $c=0$，则 $\sum c_i\alpha_i=0$ 与 $\alpha_1,\ldots,\alpha_m$ 线性无关矛盾，故 $c\neq0$，从而
\[
...
\[
 \sum_i c_i\widetilde\alpha_i=0,
\]
只看原来的前 $r$ 个分量，就有 $\sum_i c_i\alpha_i=0$。原向量组线性无关，故所有 $c_i=0$。
\end{solution}
...
\[
 \sum\limits_{i=1}^s\gamma_i=(s-1)\beta
\]
得
\[
...
把一个解 $(x,y)$ 映到
\[
\sum_{i=1}^r x_i\alpha_i=-\sum_{j=1}^s y_j\beta_j.
\]
...
\begin{enumerate}
  \item 证明
  \[
  \sum\limits_{i=1}^n(-1)^i\mu_i=0
  \]
  当且仅当 $A$ 的行向量不是该超平面的基础解系；
  \item 若
  \[
  \sum\limits_{i=1}^n(-1)^i\mu_i=1,
  \]
  求 $\mu_i$。
\end{enumerate}
\end{example}
...
为
\[
\Psi\bigl(a+(A\cap B),b+(A\cap B)\bigr)
=a+b+(A\cap B).
\]
