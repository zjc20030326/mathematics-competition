...
% ================= 中文字体:楷体粗体映射 =================
% 楷体 \textbf 真粗体补丁 (zhkai 补 BoldFont=SimHei) 的正式位置是
% elegantbook.cls 的 chinesefont=ctexfont 分支; 补丁原因、写法禁忌与警告
% 静音方式见该分支内的注释.
% 单章独立编译读到的是 TeX Live 自带的 elegantbook.cls, 本地 class 的补丁
% 不会执行, 因此这里按本地 class 是否定义了辅助宏来判断, 补做同样的处理,
% 使单章 PDF 与整书 PDF 的楷体粗体保持一致.
\ifdefined\elegantkaiboldsilence
\else
    \ifcsname setCJKfamilyfont\endcsname
        \ExplSyntaxOn
        \msg_redirect_name:nnn { xeCJK } { CJKfamily-redef } { none }
        \ExplSyntaxOff
        \setCJKfamilyfont{zhkai}[BoldFont={SimHei}]{KaiTi}
        \ExplSyntaxOn
        \msg_redirect_name:nnn { xeCJK } { CJKfamily-redef } { }
        \ExplSyntaxOff
    \fi
\fi
